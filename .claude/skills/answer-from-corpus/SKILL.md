...
2. **Read the precondition gate.** Always load three files: the task-axis index at `corpus.commons/demo/distillations/{task}/task-index.json`, the slug-table at `corpus.commons/demo/references/slug-table.json`, and the concept axis at `corpus.commons/demo/concept-index.json`. Read each whole-file in one pass (paginate with successive `offset/limit` Read calls if any file exceeds the cap; never grep or python3-query a partial slice of the JSON).

   The task-axis index partitions the task into named phases or situations (each a `section`) and lists which references apply to each (`rows` as `[need, slug-id, when]` triples); it does situation routing. The slug-table (`{schema_version, corpus, generated, next_id, slugs}`; `id → slug` under `slugs.{id}`) resolves slug-IDs to file-path keys for `references/{slug}.md`, `{slug}-deep.md`, and `distillations/{task}/{slug}-{task}.md`; `reference-index.json["refs"][id].slug` is the equivalent resolver if you've loaded the reference-index for other reasons. The concept-index does concept routing — task-index rows carry slug-IDs but not section pointers, so the concept-index is what lets you land on the named concept (Window of Tolerance, Map of Parts Work, Presence Anchor) inside the source the task-index named. Most diagnostic answers worth giving land on a named concept; the cost of loading the concept-index is one Read call, and the alternative is grepping the deep ref at answer-write time.

   **Stub-or-empty-task-index escape clause.** If `task-index.json` is a stub (`sections: []` or `generated_from: "stub-*"`), the task-axis projection has not been authored yet. In that case the matrix's task axis is empty, and the corpus-level concept axis is doing all the routing work. The precondition-gate load is unchanged (you already have concept-index in context); proceed to step 3 using concept-index to identify candidate sources rather than the task-axis rows. Read `reference-index.json` as well in this case for author/title/topic context, since the task-axis rows aren't available to disambiguate slug-IDs.

   Reference-index is otherwise optional under Protocol D — read it when the task-axis index doesn't surface a match for the situation, when the question crosses the task axis, or for the stub-escape clause above.
...
- **Precondition gate: read three corpus-level JSON indexes plus the task-axis index(es) for the domain(s) in play.**
  ...
  **Cap-aware surfaces.** Several runtime artefacts in the demo corpus either exceed or sit close to the Read tool's 25k single-call cap. Two access patterns, by file type:

  **JSON runtime indexes (`reference-index.json`, `concept-index.json`, `slug-table.json`, `lens-index.json`, `distillations/{task}/task-index.json`).** Always read the whole object. If a single Read returns the entire file, you're done; if the file exceeds the cap, paginate with successive `offset/limit` Read calls until EOF (the EOF-successive-reads pattern below applies). **Never use `python3 -c '... json.load ... .get(slug)'` or `grep` against a JSON index to extract a single key in lieu of reading the file.** The whole index is the map; loading the map in full lets later steps cross-reference between entries (aliases, neighbouring concepts, related rows in adjacent phases) without going back to disk. The structural cost of full reads is real — accept it as the price of having the whole map in context. The size table below is for sizing the pagination, not for opting into a partial read.

  Approximate sizes on the current demo corpus:
  - `slug-table.json`, `reference-index.json`, `lens-index.json`: small (single Read).
  - `concept-index.json`: ~22k tokens, fits in one Read but close to the cap; paginate if a future corpus pushes it over.
  - `distillations/{task}/task-index.json`: corpus-dependent. On the demo corpus `decision-making` and `stakeholder-engagement` exceed the cap and need pagination; `software-business` fits in one Read.

  **Markdown deep refs (`references/{slug}-deep.md`).** Several OpenStax deeps exceed the cap — nine of the 25 demo deep refs (e.g., `openstax-organizational-behavior-deep.md` at 745 lines, `openstax-principles-management-deep.md` at 1,024 lines). When Pass 3 requires the whole deep, take **successive 400-line reads until EOF**. The exact line count is in `reference-index.json`'s `refs[id].lines_deep` field; consult it before reading to know how many passes are needed. Label every read in the trace. The Source Integrity rule is firm: full coverage or labelled partial; never silent truncation.

  **EOF-successive-reads pattern (apply whenever a runtime artefact exceeds the cap, JSON or markdown):**
