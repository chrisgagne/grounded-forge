...
- `## Worked Example`: source-grounded. A short worked example showing the reference applied to a realistic scenario. The scenario itself is operator-authored, but each substantive claim about how the source's framing applies must cite the deep ref by chapter and section. **No verbatim source blockquotes in the distillation; if the source's own words are needed, link to the deep ref's `[V]` passage rather than duplicating.** The discipline: the example is yours, the framework citations within it trace back through the deep ref.
...
Distillations paraphrase by convention. Verbatim blockquotes live in the deep reference where Pass D's exactness verification has run. If the distillation needs the source's own words, link to the deep ref rather than duplicating.
...
- **Tier violation: deep-ref content in the distillation.** Distillations are derivative: they apply the source to a task, paraphrasing source claims for the task's vocabulary. Verbatim source quotes belong in the deep reference where Pass D's exactness verification has run. If the distillation needs the source's own words, link to the deep, don't duplicate.
