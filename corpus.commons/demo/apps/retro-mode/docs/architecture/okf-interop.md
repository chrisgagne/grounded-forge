...
That is deliberate. Pass I — the source-only audit behind every distillation — verifies only when it runs in a fresh, independent context; run in-context (as the demo corpus's was), a source-only self-audit certifies the producer's own errors rather than catching them. Either way it is not the independent verification OKF's `verified:` would assert, and claiming it would launder an internal check into an external assurance. The audit receipts live at corpus level in the producing repository (`docs/audit-results/`); the bundle's root `index.md` says exactly that. When the format's trust semantics and the repo's honesty standard disagree about what "verified" means, the honest reading wins.
