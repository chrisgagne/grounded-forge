...
3. **Concept axis** (`concept-index.json`) for named-concept lookups: aliases collapsed, per-source `{id, section, md_line}` pointers, cross-source coverage at a glance.
4. **Corpus catalogue** (`reference-index.json`) for named-reference / author / title / topic lookups.
...
1. Read [`reference-index.json`](../../../reference-index.json) and [`concept-index.json`](../../../concept-index.json).
2. Match the query against `concept-index.json` first (canonical names, aliases) for concept-shaped queries; against `reference-index.json` `author`, `title`, `primary_topic`, `concept_tags` for source-shaped queries. Resolve slug-IDs to file paths via [`references/slug-table.json`](../../../references/slug-table.json).
