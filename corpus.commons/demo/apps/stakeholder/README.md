...
**Measured, receipts published.** Run head-to-head against Google's OKF reference producer over the same sources, the 9-pass tier extracted 1.7× the atomic claims at hard-error rates of 0.33% vs 0.38%, no detected difference ([the round](docs/evals/rounds/2026-08-08-producer-head-to-head/)). The 2026-08-08 audit traced all 9,364 claims across its 27-source batch back to source with an independent cross-model auditor, and every verified finding was repaired ([receipts](references/_audit/)). First-query routing costs 28–47k tokens, down from ~131k ([the numbers](docs/architecture/projection-time.md)).
...
Each distillation is a markdown file at `corpus.commons/{corpus}/distillations/{task}/{slug}-{task}.md`, one source projected onto one task domain. The demo corpus ships 28 references projected onto five task axes: `decision-making` with 27 distillations, `stakeholder-engagement` with 27, `software-business` with 25, plus the two ceremony axes `aar` and `retro` with 19 each. There are 117 distillations and 23 explicit Pass G skips where a source does not warrant that task projection. Fork it and ingest your own corpus; the included content can be stripped in one command.
