...
The demo corpus predates the independence requirement — its deep references were audited in-context — so it is pending a fresh, independent re-audit and its deep references should be treated as producer-drafted until then. A single audit pass reduces but does not eliminate injected error; residual error falls with repeated, independent checks (at ingestion or at run time) and with more capable producing models.
