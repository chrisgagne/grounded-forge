...
| Need | Question |
|---|---|
| Update the board after the engagement | Has the Kanban board been updated to reflect new commitments, dropped items, scope changes, or stakeholder decisions? The board is the persistent record of the engagement. |
| Translate stakeholder asks to backlog items | When stakeholders express new needs, are they being added to the backlog at a granularity small enough to pull? |
| Surface what the team learned about engagement itself | Was anything learned about how engagement worked or did not work? Open Kanban names Retrospectives, Strategy Meetings, and Kaizen Groups as candidate forms for capturing such learnings. |
| Communicate decisions outside the room | Engagement conversations often happen with a subset of stakeholders. How are decisions being communicated to those who were not present? |
| Capture upward-correction moments | When courage was used (lower-rank corrected higher-rank), has the learning been captured? |
...
**Signal:** The Kanban board is updated by one person (a Scrum Master equivalent or a manager) rather than by the team members doing the work.
**Diagnosis:** The pull-based scheduling discipline has been short-circuited. The Respect and Communication/Collaboration values are being bypassed; the board is an observer's view, not a shared engagement artefact.
**Follow-up:** Return ownership of board updates to the people doing the work. The board's transparency value depends on team members seeing their own impact; a manager-maintained board loses that signal.
...
- A *team's engagement quality* is in question — the pull-based, courage-enabled, communicative-and-collaborative engagement Open Kanban names is not happening.
- A *push-based engagement pattern* needs naming — stakeholders are pushing work and instructions to the team rather than engaging via the board and pull-based scheduling.
...
A government-agency software team of eight has adopted Kanban without formal coaching. Six months in, the cross-functional stakeholders — two policy analysts who depend on the team's API outputs — report that they feel "in the dark." The manager schedules a status meeting. The team attends, provides verbal summaries, and the stakeholders leave satisfied. Two weeks later, the same complaint recurs.

The Kanban values diagnosis surfaces the pattern: the verbal status meeting is a push-based, one-time-event engagement. The board exists but is not accessible or visible to the policy analysts. Communication is happening (verbally, in meetings) but Collaboration is not — the analysts are not connected to the work's flow; they are receiving summaries of it.

The team redesigns the engagement using Open Kanban's visualisation practice: the Kanban board is restructured to include a "Policy-Analyst-Facing Outputs" swim lane. The policy analysts are given read access to the digital board and are shown how to read it. A weekly 15-minute "board walk" replaces the monthly status meeting — both policy analysts attend, the team walks them through the board, and questions arise from what they see rather than from prepared summaries.

Within four weeks, the policy analysts are contributing directly to backlog refinement — they can see which items are in progress, which are blocked, and can name which completions they care about most. The Courage value fires when one analyst notes that an in-progress item conflicts with a regulation that the team was not aware of; the board makes the conflict visible before code is committed.

The scenario is operator-authored. The board-as-collaboration-artefact framing, the information-radiator extension, the Communication-and-Collaboration-as-one-value principle, the pull-based scheduling discipline, and the Respect-and-Courage values are all `[V]`-marked passages in the deep reference. No verbatim source blockquotes appear in this distillation.
...
- Detailed stakeholder-mapping frameworks (Mitchell-Agle-Wood, Freeman stakeholder theory) are not developed; the source surfaces engagement *patterns* more than a stakeholder map.
- Negotiation theory (BATNA, distributive vs integrative bargaining, principled negotiation) is not developed; Open Kanban's collaboration mode is not framed as negotiation.
- Customer development methods (interviews, focus groups, lab studies) are not prescribed; the source names "customer satisfaction" as part of value but does not prescribe how customer voice enters the team.
- Specific facilitation techniques for Retrospectives, Strategy Meetings, or Kaizen Groups are not developed; these are named as candidate forms without prescription.
- Power dynamics and political analysis (French-Raven, resource dependence, coalition politics) are not developed; the Courage value names the lever for upward correction but does not analyse the power structures that make courage costly.
