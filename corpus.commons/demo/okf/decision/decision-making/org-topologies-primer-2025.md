...
description: "Krivitsky, Larman & Flemm, Org Topologies Primer, projected onto the decision-making task axis; evidence markers and attribution travel in-band."
...
---
# Krivitsky, Larman & Flemm, Org Topologies Primer — Decision-Making Distillation
...
10.  **The elevation-is-incremental decision rule.** "Elevation can be an incremental process that gradually improves the capabilities of an organization." Decisions about org change do not need to be discrete big-bang choices; the periodic re-mapping completes the learning loop. "We are never finished with the perfection journey." (p. 19, "MADE Real: (4) ELEVATE")

11.  **The named-archetype communication discipline.** When deciding org changes across stakeholders, using OT's named archetype vocabulary (CAPS-2, WHOLE-3, TASKS-1) is named as a decision-quality enabler: it lets people "describe both the current state and the desired future state of their organization with shared understanding" without resorting to "framework-specific jargon" that pre-loads the decision with assumptions. (p. 5, "Language of Org Design")

12.  **Strategy-execution alignment as a leadership decision.** "Senior management only defining business objectives" is named as a failure mode. The strategic-design stance is that leaders must also decide the internal org design — they cannot delegate the structural-fit question. The external (business objective) and internal (org design) are *one decision* tied at the leadership level. (p. 10, "Strategic Org Design")
...
### Phase 1: Framing (Recognising the org-design decision)

| Need | Question |
|---|---|
| Recognise the chain-of-fit | What business strategy/objective is this org-design decision intended to support? If we cannot name it, the decision is premature. |
...
### Phase 5: Committing (Selecting and announcing)

| Need | Question |
|---|---|
| Declare the target topology in the OT vocabulary | What named topology and named archetype set is the target? Express it precisely (e.g. "move pp. R&D from CAPS-1 + TASKS-1 to CAPS-3 teams"). |
| Anchor the decision to the business objective | Restate the business objective the chosen design is intended to deliver. The decision is a *bet* on that connection. |
...
| Need | Question |
|---|---|
| Re-MAP the current state | Has the org actually moved toward the target archetypes, or did the change stall at announcement? |
| Re-ASSESS strategic alignment | Has the business objective shifted such that the target topology is no longer the right one? |
| Surface emerging Elevating Katas | What practices that worked here are worth naming as candidate Elevating Katas for future cycles? |
| Test for new bottlenecks | Now that some bottlenecks are dissolved, which ones are now binding? AI-driven elevation often dissolves expertise constraints and surfaces coordination ones. |
| Test for mode drift back to Resource | Has the organisation drifted back toward Resource Topology under pressure for predictability? Drift toward utilisation-as-default is common. |
...
9. **Strategy-execution split.** Senior leaders defining business objectives but treating internal org design as someone else's problem. The strategic-design stance is that the same leaders own both. (p. 10)

10. **Drift back to Resource Topology under predictability pressure.** Implicit in the Primer's framing: Resource is the cultural default ("'resource managers' (or project managers) are responsible for planning and monitoring resource allocation and utilization"), so any elevation must be defended against drift back. (p. 11)

## Worked Examples (from the source)

- **The pet-supply e-commerce decision (pp. 20–21).** A pet-supply e-commerce company is in Resource topology. Leadership decides on retain-customers as the business objective, fast-flow as the org goal, and Delivery topology with CAPS-3 teams as the target. R&D is the focus of org development; the bottleneck is front-end design and coding skill. AI investment is targeted at front-end agents specifically (not generic AI adoption). Decision on AI monitoring: yes, the existing front-end designers and developers will join the CAPS-3 teams and take on agent monitoring. This is a worked decision walking through the chain-of-fit and the three AI-investment questions.

- **The running MADE example (pp. 16–19).** Starting state: WHOLE-1 product manager, CAPS-1 project-manager pool, TASKS-1 back-end developer pool, two TASKS-2 cross-functional dev+test teams. Mapping identifies Resource topology. Assessment against a "faster releases" strategic objective surfaces a strategic mismatch (Resource optimises for utilisation, not speed). Design step (worked) targets Delivery topology with cross-functional end-to-end teams. Elevation step (worked) restructures the Doing archetypes into two cross-functional end-to-end teams, moving from Resource to Delivery.

- **The market-share strategic-design example (p. 10).** Leadership decides on increasing market share in a new market as the business objective. The org goal chosen is adaptiveness (need for learning, customer interaction, easy change). The decision is to move teams vertically up (whole-business focus) and horizontally right (broader skills), targeting Adaptive Topology.

- **The SAFe and Rendanheyi framework mappings (p. 22).** SAFe is mapped as mostly TASKS-level incomplete teams plus CAPS-level fast-flow teams. RDHY is mapped as elevating thousands of micro-enterprises (Driving archetypes) that interact with users directly. Both examples illustrate that frameworks are tools that produce particular topologies; the decision is which topology, then which framework helps you get there.
...
- **Barbrook-Johnson Systems Mapping.** OT's MAP step calls for systems thinking as a discipline. Where the existing topology has feedback dynamics (e.g. utilisation pressure pushing the org back toward Resource Topology), CLD (causal-loop diagram) mapping is the disciplined complement to OT mapping. See `barbrook-johnson-systems-mapping-decision-making.md` for the CLD and System Dynamics decision-tools.
...
- The Primer is published under CC BY-NC-SA 4.0; this distillation inherits the ShareAlike obligation alongside the corpus's other SA-licensed derivatives (the Scrum Guide and Jones SA propagations).
