...
Regression modeling is the default hammer; the book prefers `glm()` over `lm()` because `glm` makes fewer assumptions about sample characteristics. The bootstrap is used everywhere closed-form alternatives are not available. The frequentist approach is used throughout, with explicit acknowledgement that "the p-value is the fall-guy of the frequentist approach". Bayesian methods are mentioned but not deployed — prior selection opens the door to bias from opinion and policy. The book explains the processes that generate Poisson, Exponential, and Negative Binomial distributions, and fits particular samples to them. Bi-exponential regressions fit fault-report duplicates well; cross-correlation reveals lag relationships in time-series data. Survival analysis (Cox proportional-hazards) is recommended when the parametric hazard distribution is uncertain.

Experimental design requires randomisation, careful subject selection, ecologically valid tasks, and control of confounding variables. Students are not representative of professional developers: students typically have 50–150 hours of programming experience vs 1,000–10,000 for professionals, and academic studies often call final-year undergraduates "experts" while industry would call them novices. Mechanical Turk subjects can stop at any time so attrition bias is a real concern. Benchmarking is full of measurement variability — SSD performance varies across runs, environment-variable size changes Perlbench performance, object-link order changes execution time, gcc release affects measured performance non-monotonically, and EC2 cloud instances show more than one distinct performance range. Goodhart's Law: any observed statistical regularity will tend to collapse once pressure is placed on it for control purposes.
...
5. **Earliest-possible fault detection is not necessarily most cost-effective.** NASA/DOD relative-cost figures are large because of the very high cost of the hardware those systems ran on.
...
<!-- Verified: ~80 substantive claims traced to deep ref. No task-application guidance (tier-separated). -->
