...
The Open Practice Library is the collective practice-knowledge of a community of practitioners working in agile delivery, DevOps, design thinking, product discovery, and team facilitation. Its central editorial framing is the **Mobius Loop**: every practice carries a `mobiusTag` of *foundation* (team and substrate), *discovery* (why and who), *options* (what to try), or *delivery* (build and measure). The library is not an alphabetical catalogue — it is a Mobius-Loop-shaped catalogue, with the per-practice tag as the first-order signal of when in the work cycle the practice applies.

The library's own *Establish Shared Principles* practice warns that practices ride on values and principles underneath, and that copying the visible practices of a successful organisation without adopting the principles that originated them does not reproduce the result. The library is a library of practices that knows the limits of practice libraries.
...
Distribution is weighted toward Foundation. Reading intent into that count — culture and substrate as where most of the practice work lives — is the reference's inference, not a stated position of the source.

## Foundation phase

Four sub-clusters:

- **Facilitation primitives.** 1-2-4-All, Affinity Mapping, Check-ins, Confidence Voting, Lean Coffee, Ice Breakers, Quote Wall, Five Whys (Taiichi Ohno, Toyota; ask why the process failed, not just why), Force Field Analysis (Lewin). Recurring shape: silent generation → small-group convergence → large-group integration. Time-boxed, participant-equalising, visual-output-producing.
...
- **Technical-substrate engineering.** Continuous Integration → Continuous Delivery → Continuous Deployment; GitOps; Everything-as-Code; Containers; Test Automation, TDD, BDD; Defence in Depth; C4 Architecture; Architectural Decision Records (ADR); Bytesize Architecture Sessions; Emerging Architecture; Wheel-of-Misfortune and Fire Drills (incident-response rehearsal). The library treats the engineering substrate as foundational, not as something added later.
- **Meta-practices.** Establish Shared Principles (the principles-over-practices contrarian); Evals (AI output quality, "evals give you confidence, not certainty"); Human in the loop (HITL/HOTL/HIC); Theory of Constraints; Westrum's Cultural Typology; Open Decision Framework and the Open Organization practices; Raison d'être; Priority Sliders; Risk Management; ROAM Boards.
...
- **Problem-space exploration.** Abstraction Laddering; Is – Is not – Does – Does not (Paulo Caroli's Lean Inception); Backcasting / Pre-mortem (Klein / Kahneman); PESTEL, Stacey Matrix, Wardley Mapping; Threat Modeling.
- **Customer and stakeholder understanding.** Empathy Mapping (Dave Gray, XPLANE); User Persona, Proto-Persona; Stakeholder Map (Power × Interest); Stakeholder Mapping (concentric circles); Stakeholder RACI Map; Stakeholders Interview; AEIOU Observation Framework; Cognitive Walkthrough; Emotional Journey; Experience Mapping; Service Blueprint; Domain Storytelling; Event Storming (Alberto Brandolini, 2013).
...
- **Ideation and prioritisation.** Crazy 8s; Silent Brainstorming; Idea Cyclone; Alternative Worlds; How Might We; Lean UX Workshop; MoSCoW; RICE Scoring Model (score Reach, Impact, Confidence, Effort; multiply the four scores).

## Options phase

Three sub-clusters:

- **Ideation.** 10-for-10; Creativity Warmups (Torrance Test of Creative Thinking); Design Sprint (5-day customer-centric process); Lean Inception; Experiment Canvas.
- **Prioritisation.** $100 Prioritisation; Impact-Effort Matrix; How-Now-Wow Matrix; Dot Voting; Eisenhower Box (urgent × important); Weighted Shortest Job First (WSJF); VUE your priorities; Kanban Priorities; Backlog Refinement.
- **Experimentation and decision framing.** Design of Experiments (hypothesis / current condition / target / obstacles / pass / measures / learning); Minimum Viable Product; Feature Toggles; Dark Launches; Split Testing (A/B); Cynefin Framework (Clear / Complicated / Complex / Chaotic / Confused-Aporetic with named responses); SWOT Analysis; Disagree and Commit (the consensus-is-not-required contrarian); Dissent Cards (Marquet's *Leadership is Language*); Example Mapping (yellow/blue/green/red four-colour sticky structure).

## Delivery phase

Three sub-clusters:

- **Cadence practices.** Scrum (3 roles / 5 events / 3 artifacts in OPL's framing); Iteration (Sprint) Planning; Daily Standup (yesterday/today/blockers; "be careful it doesn't drift into a status update"); Retrospectives (multiple formats: 4Ls, Rose Thorn Bud, Plus Minus Interesting, Realtime, Design Retro Active, Futurespective); Showcase / Sprint Review; Kanban (visualise / limit WIP / manage flow / make policies explicit / feedback loops; "Stop starting, start finishing"); Story Kick-offs; Story RePointing.
- **Reliability and safety.** Blameless Postmortem ("our job is not to point fingers"); Chaos Engineering; Risk Radar; Canary Release (caged-bird metaphor); Blue Green Deployments; Security Checklist (SafeStack).
- **Feedback and learning.** Cohort Analysis; Funnel Analysis (Ries's innovation accounting); Split Testing Multivari; Usability Testing; Guerilla Testing; Heuristic Evaluation; Agile Health Check; Burnup Chart; Code Review (McConnell on collective ownership; Atwood on egoless programming).
...
A recurring sub-heading inside many practice `howTo` bodies — it appears in about two dozen practices. The tips sections adapt the in-room facilitation shape to the distributed-team shape.
...
- *The Sprint Book* (Start At The End); Google/GV (Design Sprint variations).
- L. David Marquet, *Leadership is Language* (Dissent Cards).
...
7. **Against copying practices without principles.** Establish Shared Principles: copying practices without adopting the underlying values and principles does not reproduce the result.
...
<!-- Verified: ~80 claims traced to deep ref; no task-application guidance smuggled in (tier-separated). Task-projection material lives in the per-task distillations. -->
