<!--
Converted markdown for: Open Kanban — Open Source Initiative to create a Kanban core that is Agile, Lean and Free
Source: github.com/agilelion/Open-Kanban (Joseph Hurtado, AgileLion Institute)
Release: 1.00 Rev A | Licence: CC BY 3.0 Unported

Ingest pattern: repository-as-source. The Open Kanban repository contains a
top-level README.md (the GitHub-facing canonical view) and a parallel
"Open Kanban Main v1_RevA.md" inside Open Kanban Support Documents/ (the
stable bundled version). Those two files are textually identical apart from
a single typo difference. The repository also contains diagrams (PNG/PDF, not
inlined here) and six translation directories (French, Italian, Portuguese,
Russian, Spanish, Ukrainian) which are excluded from this English-only
conversion. The "Open Kanban Translations/README.md" call-for-translators
page is appended as Appendix A below.

This converted markdown is the deep-read source for Pass C. All section names
are preserved exactly as in the source. The single substantive prose change
from the underlying files is the section heading prefix "# Open Kanban Main
Document" added at the top of the canonical text below, so that the
appendix is structurally distinguishable.
-->

# Open Kanban Main Document
...
# Appendix A: Open Kanban Translations (call for translators)
...
# Appendix B: Repository diagram files (referenced, not inlined)

The Open Kanban repository ships the following diagram files (binary, not converted to text):

- `Open Kanban Diagrams/Kanban-Board-Example-AgileLion.png` — example Kanban board, CC BY by AgileLion Institute.
- `Open Kanban Diagrams/Open Kanban and Derivative Methods - Source in Illustrator -CC-BY-Joseph Hurtado AgileLion Institute.pdf` — source-format diagram of the Open Kanban Methods ecosystem.
- `Open Kanban Diagrams/Open-Kanban-and-Derivative-Methods---Source-in-Illustrator--CC-BY-Joseph-Hurtado-AgileLion-Institute.png` — rasterised version of the same diagram.
- `Open Kanban Diagrams/What-is-Kanban-Diagram-CC-BY-Joseph-Hurtado-AgileLion-Insitute.png` — diagram explaining "what is Kanban".
- `Open Kanban Diagrams/Creative Commons Icons/` and `Open Kanban Diagrams/Creative Commons Photos/` — supporting CC iconography and photography assets.

The Markdown body of the README also references the now-defunct Flickr image `http://farm8.staticflickr.com/7420/9502339079_7c08751a30_o.png` as the ecosystem diagram and a Dropbox-hosted Kanban board image at `https://dl.dropboxusercontent.com/u/73847422/Kanban-Board/Kanban-Board-Example-AgileLion.png`. These external image hosts may not resolve at any given time; the repository's `Open Kanban Diagrams/` folder holds the canonical local copies.

---

# Appendix C: Files excluded from this conversion

The following English-language source-of-record duplicate was identified during conversion:

- `Open Kanban Support Documents/Open Kanban Main v1_RevA.md` (and its PDF rendering) — substantively identical to the top-level README.md, with only a single typo difference ("submisssions" with three s's in the support-doc copy versus "submissions" in the README; the converted markdown above carries the README's spelling).

The repository also contains six translation directories (French, Italian, Portuguese, Russian, Spanish, Ukrainian) under `Open Kanban Translations/`. Translations are excluded from this conversion: the methodology content is identical to the English source, and a corpus that ships all six translations would inflate without adding methodological content. The translations remain in the original repository tree at `sources/original/open-kanban-repo/Open Kanban Translations/` for any later translation-specific work.
