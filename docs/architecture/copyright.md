...
3. **LENS-INDEX regeneration.** The per-app `LENS-INDEX.md` is generated from the filtered lens list, not copied verbatim from the hand-authored source. Excluded lenses simply don't appear. The hand-authored source `LENS-INDEX.md` in `corpus.commons/demo/lenses/` remains the seed catalogue; it lists every lens regardless of visibility and is not used at runtime by apps.
