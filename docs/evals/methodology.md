...
The exception is the OKF round, which ran entirely on public artifacts and is published whole — captures, decoded judge runs, citation audits, and error exhibits — at [`rounds/2026-07-28-okf/`](rounds/2026-07-28-okf/). It holds the corpus, consumer, and judge constant and varies only the producer, across four arms. Headline: every arm validates as conformant OKF and the arms rank within points of each other under blind preference judging; traced source-level fidelity is the separating instrument — zero unsupported claims in the audited arms, zero by construction in the deterministic baseline, and 9.4% fabricated-or-contradicted notes in the un-audited LLM-minted arm, including one confirmed chain where an invented claim propagated into a cited answer and passed the answer-level citation audit, invisible without source-level tracing.
