...
The methodological point that separates producers of the same corpus is source-level fidelity tracing — tracing each claim back to the source text — not format conformance (a container property) or blind preference ranking (fluency). An injected claim can pass an answer-level citation audit and still be a fabrication; it surfaces only when you trace to the source. That tracing is what an ingestion audit is, and — per [`source-integrity.md`](../architecture/source-integrity.md) — it only verifies when the audit is run independently of the producer, not in the producer's own context.
