...
- ~60–90 minutes wall-clock, depending on how many of the demo's 27 references fire applicable for your task.
...
For the demo corpus's 27 references and an axis like `risk-assessment`, expect roughly 8-15 *yes* outcomes. The remaining sources are genuinely off-axis: a marketing reference has little to say about risk-assessment in the operational sense, even though *risk* appears in its text.
...
Cost discipline: the per-distillation orchestration is the expensive step ($2–8 of Opus tokens for the demo's 27 references). The scoping and scaffolding steps are cheap ($0.10–0.50 each). The cost concentrates where the value concentrates: in the actual projections.
